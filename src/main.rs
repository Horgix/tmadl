...
enum TmadlSubcommands {
    ...
    Upload {
        ...
        #[arg(
            ...
            help = "Path to the local recording file"
        )]
        path: String,
    },
    ...
    Ask {
        ...
        #[arg(
            long,
            value_name = "TRANSCRIPTION",
            help = "The transcription to interrogate"
        )]
...
fn main() {
    ...
    match &cli.command {
        ...
        Some(TmadlSubcommands::Upload { path }) => {
            ...
            let store = S3RecordingStore::new(&s3_bucket);
            store
                .send_local_recording(&recording.unwrap(), path)
                .unwrap();
        }
        Some(TmadlSubcommands::Ask { transcription }) => {
            println!("Interrogating transcription: {}", transcription);
            let mock_recording = Recording {
                id: transcription.clone(),
                source: Some("PLACEHOLDER".to_string()), // FIXME
                date_time: Some(chrono::Utc::now()),     // FIXME
                duration: Some(3600),                    // FIXME
                number_of_speakers: Some(2),             // FIXME
                language: Some("French".to_string()),    // FIXME
            ...
            let input = get_prompt(
                domain::summary::SummaryRequest {
                    ...
                    additional_context: Some(vec!["FIXME_THIS_IS_A_PLACEHOLDER".to_string()]),
                },
                transcript,
            );
